...
run()
{
	...
	do
		open_terminal "python3 python/train_agent.py $env $agent --nsteps $steps --tcp_rank $j --tcp_ports $port_string --train_prop $trainprop" &
		sleep 0.4
	done
	sleep 5
	python3 python/train_agent.py $env $agent --nsteps $steps --tcp_rank 0 --tcp_ports $port_string --train_prop $trainprop
}
